...
- [x] **Alerta de acumulado alto por e-mail** — dispara via Resend quando a estimativa do próximo concurso cruza `MEGASENA_LIMIAR_ALERTA` (padrão R$100mi), só na transição abaixo→acima (não repete a cada corrida do cron). **Pendente de configuração**: rodar `npx convex env set RESEND_API_KEY <chave>` e `npx convex env set MEGASENA_ALERTA_EMAIL <destino>` no deployment do Convex — sem isso o alerta fica só um `console.warn` no log.
